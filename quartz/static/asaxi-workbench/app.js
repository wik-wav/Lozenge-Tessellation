...
const browserWorkerPath = String(runtimeConfig.browserWorker || "").trim();
...
class BrowserRuntimeClient {
  constructor(path) {
    ...
    this.worker = new Worker(new URL(path, document.baseURI).href);
    this.worker.addEventListener("message", (event) => {
      ...
      if (response.ok) {
        ...
        pending.reject(
          new Error(response.error || "Browser translation failed"),
        );
      }
    });
    this.worker.addEventListener("error", (event) => {
      const error = new Error(
        event.message || "Browser translation worker failed",
      );
...
const browserRuntime = browserWorkerPath
  ? new BrowserRuntimeClient(browserWorkerPath)
  : null;
...
const state = {
  ...
  translationAvailable: Boolean(apiBase),
  phraseSearchTimer: 0,
...
const elements = {
  ...
  interpretationsDisclosure: document.querySelector(
    "#interpretations-disclosure",
  ),
...
function focusedClause() {
  return (
    currentClauses().find((clause) => clause.id === state.focusedClauseId) ||
    null
  );
}
...
function selectedAlternative(clause = focusedClause()) {
  ...
  const selectedId =
    state.selectedByClause.get(clause.id) ||
    clause.translation.selected_alternative_id;
  const choices = availableInterpretations(clause);
  return (
    choices.find((alternative) => alternative.id === selectedId) ||
    choices[0] ||
    null
  );
}
...
function composeOutput() {
  ...
  state.document.clauses.forEach((clause) => {
    ...
    output +=
      ...
      clause.translation.fallback_output_text ||
      clause.source_text;
...
function renderOutput() {
  ...
  elements.outputRomanizationRow.classList.toggle(
    "is-inactive",
    !outputIsAsaxi,
  );
...
function updateSourcePreview() {
  ...
  elements.sourceRomanizationRow.classList.toggle(
    "is-inactive",
    !sourceIsAsaxi,
  );
...
function setDirection(direction, { reset = true } = {}) {
  ...
  elements.sourceLabel.textContent = sourceIsAsaxi
    ? "Source · Asaxi"
    : "Source · English";
  elements.outputLabel.textContent = sourceIsAsaxi
    ? "Output · English"
    : "Output · Asaxi";
  elements.source.classList.toggle("asaxi-editor", sourceIsAsaxi);
  elements.output.classList.toggle("asaxi-editor", !sourceIsAsaxi);
  elements.source.placeholder = sourceIsAsaxi
    ? "JOHN apo chỏnů."
    : "John eats an apple.";
...
function statusLabel() {
  ...
  const bestEffortCount = state.document.clauses.filter(
    (clause) =>
      clause.translation.status === "unsupported" &&
      clause.translation.fallback_output_text,
  ).length;
  ...
  if (authoredSenseChoices) {
    return [
      "ambiguous",
      `Choose an authored sense · ${plural(authoredSenseChoices, "clause")}`,
    ];
  }
  ...
  if (state.document.status === "partial" && authoredCount) {
    return [
      "partial",
      `Authored output available · ${plural(authoredCount, "clause")}`,
    ];
  }
  ...
  if (state.document.status === "ambiguous") {
    return [
      "ambiguous",
      `Review available · ${plural(counts.review, "clause")}`,
    ];
  }
  if (state.document.status === "partial") {
    if (bestEffortCount) {
      return [
        "partial",
        `Best effort · ${plural(bestEffortCount, "clause")} needs review`,
      ];
    }
    return [
      "partial",
      `Partial translation · ${plural(counts.unsupported, "unsupported clause")}`,
    ];
  }
  if (bestEffortCount) {
...
function renderClauseList() {
  ...
  clauses.forEach((clause) => {
    ...
    const bestEffort =
      status === "unsupported" && clause.translation.fallback_output_text;
...
function interpretationCaption(alternative) {
  ...
  const selected = alternative.ambiguities
    .map((ambiguity) => ambiguity.selected)
    .filter(Boolean);
  if (selected.length) {
...
function renderInterpretations() {
  ...
  const hasAuthored = (clause?.translation.authored_matches || []).length > 0;
  elements.interpretationsDisclosure.hidden =
    alternatives.length < 2 && !hasAuthored;
  ...
  elements.interpretationsCurrent.textContent =
    selected?.output_text || "No selection";
  elements.interpretationsCount.textContent = plural(
    alternatives.length,
    "output",
  );
...
function renderDiagnostics() {
  ...
  if (!currentClauses().length) {
    ...
    diagnostics.forEach((diagnostic) => {
      ...
      const location =
        diagnostic.token_index === null
          ? ""
          : ` Token ${diagnostic.token_index + 1}:`;
...
function renderTokenTable() {
  ...
  const interlinearByIndex = new Map(
    interlinear.map((record) => [record.token_index, record]),
  );
  ...
  tokens.forEach((token) => {
    ...
    const values = [
      ...
      record?.morphemes?.join(" + ") ||
        analysis?.morphemes?.join(" + ") ||
        token.surface,
    ];
    ...
    features.appendChild(
      featureElements(record?.features || analysis?.features || {}),
    );
...
function renderIssues() {
  ...
  elements.reviewCount.textContent = String(
    assumptions.length + ambiguities.length,
  );
...
function applyDocument(payload) {
  ...
  payload.clauses.forEach((clause) => {
    const selected =
      clause.translation.authored_matches?.[0]?.id ||
      clause.translation.selected_alternative_id;
    if (selected) {
  ...
  state.focusedClauseId =
    payload.selected_clause_id || payload.clauses[0]?.id || null;
...
async function translate() {
  ...
  try {
    ...
    if (browserRuntime && !apiBase) {
      payload = await browserRuntime.request(
        "translate-document",
        requestPayload,
        {
    ...
    if (state.requestController === controller) {
      ...
      elements.translate.disabled = !state.translationAvailable;
    }
  }
}
...
function staticPhraseSearch(entries, query) {
  ...
  return entries
    ...
    .map((entry) => {
      ...
      const caseInsensitive = forms.find(
        (form) => !exact && phraseSearchKey(form) === needle,
      );
      const sourceMatch = forms.find((form) =>
        phraseSearchKey(form).includes(needle),
      );
    ...
    .sort(
      (left, right) =>
        ...
        left.source_text.localeCompare(right.source_text),
    )
    .slice(0, 50);
}
...
function renderPhraseResults(results, { unavailable = false } = {}) {
  ...
  results.forEach((result) => {
    ...
    meta.textContent =
      matchDescriptions[result.match_kind] || "Reviewed phrase translation";
...
async function loadHealth() {
  try {
    ...
    let payload;
    if (browserRuntime && !apiBase) {
      setStatus("loading", "Loading translator");
      payload = await browserRuntime.request("health");
    } else {
    ...
    elements.phrasebookCount.textContent = plural(
      phrases.verified_count || 0,
      "entry",
      "entries",
    );
    if (phrases.writable) {
    ...
    elements.databaseStatus.textContent = browserRuntime
      ? "Translator runtime unavailable"
      : "Static phrasebook mode";
    elements.translate.disabled = true;
    elements.translate.title = browserRuntime
      ? "The published grammar runtime could not be loaded"
      : "Full translation requires the Asaxi Workbench API";
    if (browserRuntime) {
      setStatus("error", error?.message || "Translator runtime unavailable");
    }
    try {
      const entries = await loadPublicPhraseIndex();
      elements.phrasebookCount.textContent = plural(
...
function focusClauseFromSelection() {
  ...
  const clauses = currentClauses();
  let candidate = clauses.find(
    (clause) => start >= clause.span.start && start < clause.span.end,
  );
  if (end > start) {
    candidate = clauses
      .map((clause) => ({
        ...
        overlap: Math.max(
          0,
          Math.min(end, clause.span.end) - Math.max(start, clause.span.start),
        ),
      }))
...
function announceFrameHeight() {
  ...
  frameHeightRequest = requestAnimationFrame(() => {
    ...
    window.parent.postMessage({ type: "asaxi-workbench-height", height }, "*");
  });
}

if (window.ResizeObserver) {
  const frameObserver = new ResizeObserver(announceFrameHeight);
  frameObserver.observe(document.documentElement);
  frameObserver.observe(document.body);
}
window.addEventListener("load", announceFrameHeight);
window.addEventListener("resize", announceFrameHeight);
