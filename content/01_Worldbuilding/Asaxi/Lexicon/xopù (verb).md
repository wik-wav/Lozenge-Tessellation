---
freq: 41
title: xopù (verb) - to move downward
Word (Asaxi): xopù
pitch_accent: H.L
pitch_accent_class: lexical
trnsltion. En: to move downward, to descend, to sink, to drop (oneself)
trnsltion. Pl: schodzić, opadać, zniżać się
Transitivity: intransitive, transitive
tags:
  - Asaxi
  - language
  - verb
---
# xopù ([[02_Asaxi Verbs_Root (List)]])

- - -

<span class="asaxi-script">xopù</span>

<span class="asaxi-script-alpha">xopù</span>
### Transitivity / Valency
**Ambitransitive**
* **Intransitive:** To descend, to go down, to sink.
* **Transitive:** To lower, to bring down.

### Lexical Aspect
- **Durative (Activity)** — unfolds over time.
- **Aspect markers**: **na-** "do repeatedly" · **tå-** "do once" · **ni-** "begin to" · **chå-** "do fully".

### Grammatical Note
**[[02_Asaxi Verbs_Root (List)]]:** Compound root verb.

### Semantic Field
[[Smntc_Field Motion]]

### Pronunciation
IPA: /ɦopɯ/

### Pitch Accent

- **Morae:** <span class="asaxi-text">xo · pù</span>
- **Pattern:** `H · L`
- **Class:** `lexical`

### Translations
- English: to descend, to lower, to go down, to sink
- Polish: schodzić, opuszczać, zniżać

### Example sentence

#### Example 1

- **Asaxi:** to wo [blinds] xopù.
- **English:** I lower the blinds.
- **Polish:** x

### Alternative Forms
Null

### Etymology
**xo** (Move) + **[[pù- (relational locative) (particle)|pù]]** (Down).

### Synonyms
Null

### Antonyms
- **xona** (To ascend).

### Derived terms
Null

### Usage Note
Distinct from **[[topù (verb)|topù]]** (To drop).
* **topù:** Instantaneous release (Gravity takes over).
* **xopù:** Controlled downward movement ("I lower the blinds").
