---
title: mi- (Tense Prefix) - "Currently feeling", "Sensing right now", "Internally experiencing".
Word (Asaxi): mi-
...
* **Phonotactics (The `-j-` Bridge):** If the verb root begins with a vowel, the bridge **-j-** is inserted to maintain the palatal quality.
    * *Rule:* `mi` + `V` $\rightarrow$ **mijV**.
    * *Example:* `mi` + `ůchů` $\rightarrow$ **mijůchů**.
...
Sound symbolism for internal/soft state (Nasals). Parallel to **[[sỏ (Tense Prefix)|sỏ-]]** (Subjective Past).
...
- **[[sỏ (Tense Prefix)|sỏ-]]** (Subjective Past / Memory).
