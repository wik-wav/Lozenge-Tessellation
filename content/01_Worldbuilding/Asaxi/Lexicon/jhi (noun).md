---
title: jhi (noun) - hour
Word (Asaxi): jhi
pitch_accent: H
pitch_accent_class: lexical
trnsltion. En: hour
trnsltion. Pl: godzina
tags:
  - Asaxi
  - language
  - noun
  - vocab_expansion
---
# jhi ([[01_Asaxi Nouns (List)]])

- - -

<span class="asaxi-script">jhi</span>

<span class="asaxi-script-alpha">jhi</span>

### Noun class (warm / cold)

**Cold**

### Pronunciation

IPA: /d̠ʒi/

### Pitch Accent

- **Morae:** <span class="asaxi-text">jhi</span>
- **Pattern:** `H`
- **Class:** `lexical`

### Semantic Field

[[Smntc_Field Time]]

### Translations

- English: hour
- Polish: godzina

### Example sentence


#### Example 1

- **Asaxi:** nixå tam jhiwa onă bỏhjá ni xő zèsùkixů.
- **English:** For two hours the Boy loved him.
- **Polish:** x

### Alternative Forms

x

### Etymology

Loan < Japanese *ji* (時). Fills the unit gap between åkamm (moment) and xi (day).

### Synonyms

x

### Antonyms

Null

### Derived terms

Null

### Usage Note

Duration rule: time units join [[nixå (particle)|nixå]] for duration readings (`nixå tam jhiwa` "for two hours") but stand alone as ordinary nouns elsewhere. See [[41_Time & Frequency (The `mă` System)]].

First attested in [[onă gaksamipỏpỏ (The Velveteen Rabbit)]].

