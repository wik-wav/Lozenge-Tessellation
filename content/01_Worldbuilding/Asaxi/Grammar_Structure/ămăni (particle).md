---
title: ămăni (particle) - at your leisure
Word (Asaxi): ămăni
pitch_accent: L.L.L
pitch_accent_class: atonal
trnsltion. En: at your leisure
tags:
  - Asaxi
  - language
  - grammar
---
# ămăni ([[02_Particles in Asaxi]])

- - -

<span class="asaxi-script">ămăni</span>

<span class="asaxi-script-alpha">ămăni</span>
### Grammatical function

- **Type:** Temporal Adverb / Phrase
- **Function:** **Permissive Time**
- **Meaning:** "At your leisure", "Whenever you feel like", "When the time is right for you".

### Usage Note

Used to give the listener total freedom over the schedule. It implies "It doesn't matter to me; do it when it suits you."

### Pronunciation

IPA: /aɪmaɲi/

### Pitch Accent

- **Morae:** <span class="asaxi-text">ă · mă · ni</span>
- **Pattern:** `L · L · L`
- **Class:** `atonal`

### Example sentence


#### Example 1

- **Asaxi:** no ămăni shěsonů kă.
- **English:** Please read whenever you feel like it.])]
- **Polish:** x

### Etymology

**[[ămă (noun)|ămă]]** (Right Time) + **[[ni (particle)|ni]]** (Towards).
*Lit:* "Towards the right time." (Move towards the moment that feels right to you).

### Synonyms

- **[[gămă (particle)|gămă]]** (Whenever / At any time - more neutral/random).

### Derived terms

- [[ămă (noun)|ămă]] - base particle
