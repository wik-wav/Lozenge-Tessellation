---
freq: 36
title: gabåbwoshá (noun) - strongman, powerlifter, one with a bear-like physique
Word (Asaxi): gabåbwoshá
trnsltion. En: strongman, powerlifter, brute, one with a bear-like physique
trnsltion. Pl: siłacz, mocarz, niedźwiedź (typ budowy)
tags:
  - Asaxi
  - language
  - noun
  - ga-noun
  - ga-literal
---
# gabåbwoshá ([[00_Ga-noun Compounds in Asaxi (list)|00_Ga-noun Compounds in Asaxi (list)]])

- - -

<span class="asaxi-script">gabåbwoshá</span>

<span class="asaxi-script-alpha">gabåbwoshá</span>
### Noun class (warm / cold)
**Warm**

- Reasoning: Animate being.

### Pronunciation
IPA: /ɡabau̯bʷoʃɑ/

### Semantic Field
[[Smntc_Field The Body]] / [[Smntc_Field Animals]]

### Translations
- English: strongman, powerlifter, brute, bear (physique)
- Polish: siłacz, mocarz, niedźwiedź (typ budowy)

### Example sentence

#### Example 1

- **Asaxi:** to gabåbwoshá xădănă xèro ronů.
- **English:** The strongman lifts very heavy weights.
- **Polish:** x

### Alternative Forms
Null

### Etymology
**[[ga (particle)|ga]]** (Type) + **[[båbå (noun)|båbå]]** (Muscle) + **[[bwo (noun)|bwo]]** (Fat) + **[[shá (noun)|shá]]** (Creature).
*Lit:* "Muscle-Fat-Type-Creature."

### Synonyms
Null

### Antonyms
Null

### Derived terms
- **gabåbwoshánă / gabåbwoshánýj** (adjectival forms) *(potential)*

### Usage Note
Describes a creature possessing both high muscle mass (`båbå`) and significant body fat (`bwo`). It implies functional, bulky strength ("Bear Mode"), distinct from the lean/defined strength of `gabåbåshá` (Bodybuilder) or the pure fatness of `gabwoshá`.

**Animal Representatives:**

1.  **[Bear]** (The archetype of muscle+fat).
2.  **[Hippo]** (Deceptively fat but immensely strong).
