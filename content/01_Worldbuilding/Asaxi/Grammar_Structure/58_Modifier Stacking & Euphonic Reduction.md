---
title: 58_Modifier Stacking & Euphonic Reduction
tags:
  - Asaxi
  - language
  - grammar
...
- _Analysis:_ `waonă` (Homely) → **`waona`** (Reduced) + `pjonă` (Bright) + `pja` (Lights).
- _Meaning:_ "Bright homely lights."

**Example (Cold Chain):** `goný shěsonýj shěso`

- _Analysis:_ `gonýj` (Heavy/Big) → **`goný`** (Reduced) + `shěsonýj` (Smart) + `shěso` (Book).
- _Meaning:_ "A big, smart book."
...
- **Logic:** The object is composed of _all_ these elements simultaneously.

**Example:** `gadăbwojohn`

- _Analysis:_ `ga` + `dă` (Immensity) + `bwo` (Fat) + `John`.
- _Meaning:_ "Big Fat John" (A specific constitutional type of John).

**Example:** `gapogo`

- _Analysis:_ `ga` + `apo` (Red) + `go` (Rock).
- _Meaning:_ "Red rock."

### **3. Mixed Modification (Simile + Constitution)**

When using both types of modifiers, the **Qualitative Adjectives** (Simile) must come _before_ the **Constitutional Compound** (Fact).

- **Order:** `[Reduced Adj]` ... `[Full Adj]` + `[Ga-Compound]`

**Example:** `goný găxiŕonă gapogo`

- _Analysis:_ `goný` (Hard/Rock-like [Reduced]) + `găxiŕonă` (Vibrant) + `gapogo` (Red-Rock).
- _Meaning:_ "A hard, vibrant red-rock."
