---
freq: 88
title: kjèmbinů (verb) - to play (games), to scheme
Word (Asaxi): kjèmbinů
trnsltion. En: to play (games), to scheme, to plot, to trick, to hustle
trnsltion. Pl: grać, kombinować
Transitivity: intransitive, transitive
tags:
  - Asaxi
  - language
  - verb
---
# kjèmbinů ([[02_Asaxi Verbs_ů (List)]])

- - -

<span class="asaxi-script">kjèmbinů</span>

<span class="asaxi-script-alpha">kjèmbinů</span>
### Transitivity / Valency
**Intransitive** (Playing/Scheming) or **Transitive** (Playing a specific game).

### Lexical Aspect
- **Durative (Activity)** — unfolds over time.
- **Aspect markers**: **na-** "do repeatedly" · **tå-** "do once" · **ni-** "begin to" · **chå-** "do fully".

### Semantic Field
[[Smntc_Field Behaviour]] / Entertainment / Deception

### Pronunciation
IPA: /kʲəmbinuw/

### Translations
- English: to play (games), to scheme, to plot, to trick, to hustle
- Polish: grać, kombinować

### Example sentence
**Literal:**
**To wo igùŕa kjèmbinů.**
_I play a game._

**Figurative:**
**To John kjèmbinů.**
_John is scheming / up to something._

### Alternative Forms
Null

### Etymology
"To perform the function of cards" (Playing/Gambling).

### Synonyms
Null

### Antonyms
Null

### Root Noun
- [[kjèmbi (noun)|kjèmbi]]

### Derived terms
Null

### Grammatical Note
Derived from **[[kjèmbi (noun)|kjèmbi]]** (Cards) + **[[06A_-n- (Morphological Infix)|-n-]]** (Performance/Use) + **-ů**.
* *Lit:* "To use cards."

### Usage Note (Double Meaning)
1.  **Literal:** To play a game (Video games, cards, board games).
2.  **Figurative:** To play *games* with people (Scheming, manipulating, "Kombinować").
