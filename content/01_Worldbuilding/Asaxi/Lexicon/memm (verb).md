---
title: memm (verb) - to close, to shut
Word (Asaxi): memm
trnsltion. En: to close, to shut, to seal
trnsltion. Pl: zamykać
Transitivity: ambitransitive
tags:
  - Asaxi
  - language
  - verb
---
# memm ([[02_Asaxi Verbs_Root (List)]])

- - -

<span class="asaxi-script">memm</span>

<span class="asaxi-script-alpha">memm</span>
### Transitivity / Valency
**Ambitransitive**
To close / To be closed.

### Lexical Aspect
- **Punctual (Achievement)** — happens in a single moment.
- **Aspect markers**: **na-** "do repeatedly" · **tå-** "do once" · **ni-** "begin to" · **chå-** "do fully" · **-ů** "make it ongoing".

### Grammatical Note
**[[02_Asaxi Verbs_Root (List)]]:** Root Verb.

### Semantic Field
[[Smntc_Field Motion]] / Action

### Pronunciation
IPA: /me.m̩/
*Note:* Two syllables. The final `mm` functions as a syllabic nasal nucleus.

### Translations
- English: to close, to shut, to seal
- Polish: zamykać

### Example sentence
**Kem zèmemm.**
_The door closed._

### Alternative Forms
Null

### Etymology
Sound symbolism: The bilabial `m` closes the mouth, mimicking the shutting action.

### Synonyms
- **[[chěcho' (verb)|chěcho']]** (To close/shut - emphatic/forceful).

### Antonyms
- **em.mo** (To open).

### Derived terms
- **memmshá** (agent noun) *(potential)*
- [[memno (noun)|memno]] - button
