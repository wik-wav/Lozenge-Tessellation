---
title: xůxů (verb) - to forget, to lose (a memory)
Word (Asaxi): xůxů
trnsltion. En: to forget, to lose (a memory), to slip one's mind
trnsltion. Pl: zapomnieć
Transitivity: transitive, intransitive
tags:
  - Asaxi
  - language
  - verb
---
# xůxů ([[02_Asaxi Verbs_Root (List)]])

- - -

<span class="asaxi-script">xůxů</span>

<span class="asaxi-script-alpha">xůxů</span>
### Transitivity / Valency
**Transitive** (Forgetting X) or **Intransitive** (Being forgetful).

### Lexical Aspect
- **Punctual (Achievement)** — happens in a single moment.
- **Aspect markers**: **na-** "do repeatedly" · **tå-** "do once" · **ni-** "begin to" · **chå-** "do fully" · **-ů** "make it ongoing".

### Grammatical Note
**[[02_Asaxi Verbs_Root (List)]]:** Compound root verb.

### Semantic Field
[[Smntc_Field Perception]] / Cognition

### Pronunciation
IPA: /ɦuu̯ɦuu̯/

### Translations
- English: to forget, to lose (a memory), to slip one's mind
- Polish: zapomnieć

### Example sentence
**To wo [name] xůxů.**
_I forget the name._

### Alternative Forms
Null

### Etymology
Reduplication related to **[[xoxo (verb)|xoxo]]** (To depart).
*Logic:* "It departs-departs." (The thought leaves the mind).

### Synonyms
- **[Lose]** (TBD).

### Antonyms
- **[Remember]** (TBD - likely related to `ma` "have" or `hù` "back").

### Derived terms
- **xůxůshá** (agent noun) *(potential)*
