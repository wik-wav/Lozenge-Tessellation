---
title: kăgo (Interrogative Quantifier) - how much?, how many?
Word (Asaxi): kăgo
trnsltion. En: how much?, how many?
trnsltion. Pl: ile?
tags:
  - Asaxi
  - language
  - particle
  - interrogative
---
# kăgo ([[25_Interrogative Pronouns (Wh-Words)]])

- - -

<span class="asaxi-script">kăgo</span>

<span class="asaxi-script-alpha">kăgo</span>
### Grammatical function

- **Type:** Interrogative Quantifier (k-word)
- **Function:** Asks for **quantity or amount**.
- **Meaning:** "How much?", "How many?"

### Usage Notes

- Completes the k-word paradigm: *kjo* (what), *kshá* (who), *ksi* (where), *ksá* (how), *ksè* (why), *kvå* (when), **kăgo** (how many).
- Behaves like the **-go quantifiers** ([[dăgo (Quantifier)|dăgo]], [[hjigo (Quantifier)|hjigo]], [[fůtăgo (Quantifier)|fůtăgo]]): same syntactic slots, interrogative force.
- In-situ like all k-words; no word-order change: **Kăgo jå kè?** ("How many do you want?")

### Pronunciation

IPA: /kaɪgo/

**Formation note:** the bare cluster \**kg-* is phonotactically illegal, so the interrogative *k-* takes an epenthetic *ă* before the quantifier base *-go*.

### Example sentence

**O-chỏno kăgo?**
_"How much (is) this food?"_

### Etymology

**k-** (interrogative formative, cf. [[25_Interrogative Pronouns (Wh-Words)]]) + **-go** (quantifier base of *dăgo/hjigo/fůtăgo*) with epenthetic *ă*.

### Synonyms

- [[kjo (Interrogative Pronoun)|kjo]] (what — general k-word)

### Antonyms

- [[dăgo (Quantifier)|dăgo]], [[hjigo (Quantifier)|hjigo]] (declarative quantity answers)

### Derived terms

- **-go** quantifier family: [[dăgo (Quantifier)|dăgo]], [[hjigo (Quantifier)|hjigo]], [[fůtăgo (Quantifier)|fůtăgo]]
